...
const Cell = ({id, color}) => {
  ...
  const handlePress = useCallback(
    (playerNo, pieceId) => {
      console.log('Pressed:', playerNo, pieceId);
  ...
  return (
    <View
      ...
      {isStartSpot && (
        <Ionicons name="star-outline" size={RFValue(15)} color="gray" />
      )}
      {isArrowSpot && (
        <Ionicons
      ...
      {piecesAtPosition.map(piece => (
        <View
          key={piece.id}
          style={[styles.pieceContainer, {backgroundColor: color}]}
          onPress={() => handlePress(piece.playerNo, piece.id)}>
          <Text>{piece.id}</Text>
        </View>
      ))}
    </View>
